...
class ABCROWN:
    ...
    def incomplete_verifier(
        ...
        if len(specs) > 1:
            ...
            rhs = torch.tensor(specs[0][1], dtype=data.dtype, device=torch.device("cpu")).unsqueeze(0)
            stop_func = stop_criterion_batch_any(rhs)

        model = LiRPANet(model_ori, in_size=data.shape, c=c, device='cpu')
    ...
    def main(self, interm_bounds=None):
        ...
        for new_idx, csv_item in enumerate(example_idx_list):
            ...
            device = torch.device("cpu") # I added it
